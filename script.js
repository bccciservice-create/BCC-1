...
const SUPABASE_URL = "https://lojlixnpzqjfsampcijx.supabase.co";
const SUPABASE_ANON_KEY = "sb_publishable_szkbcqtYw3iYgHUXVutLhg_AF0T89rR";
...
document.getElementById("requestRegisterOtpBtn")?.addEventListener("click",async()=>{
...
alert("Entrez d'abord une adresse email valide.");
return;
}
if(mobileMoney.length < 8){
alert("Entrez d'abord un numero Mobile Money valide.");
return;
}
...
alert("Ce numero Mobile Money existe deja.");
return;
}
pendingRegisterOtp = String(Math.floor(100000 + Math.random() * 900000));
status.innerText = `Code OTP lance. Mode GitHub test : ${pendingRegisterOtp}`;
showSupabaseStatus(await supabaseInsert("password_otp_requests",{
email,
mobile_money:mobileMoney,
otp_hash:pendingRegisterOtp,
status:"pending",
expires_at:new Date(Date.now() + 10 * 60 * 1000).toISOString(),
created_at:new Date().toISOString()
}),"le code OTP inscription");
});
...
if(!pendingRegisterOtp || otp !== pendingRegisterOtp){
alert("Code OTP invalide. Cliquez d'abord sur Lancer le code OTP puis entrez le code affiche.");
return;
}
...
pendingRegisterOtp = "";
document.getElementById("registerOtpStatus").innerText = "Inscription validee avec OTP.";
